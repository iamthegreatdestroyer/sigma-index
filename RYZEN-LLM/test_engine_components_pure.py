...
Pure Python implementation for testing logic without C++ extensions
"""
...
def test_matmul_correctness():
    """Test that naive matmul produces correct results"""
    print("\nTesting matrix multiplication correctness...")

    try:
        # Small matrices for manual verification
        M, N, K = 2, 3, 2

        # Create simple ternary weights
        ternary_weight = TernaryWeight(K, N, 0)
        # Set weights manually: [[1, -1, 0], [0, 1, -1]]
        ternary_weight.values = [1, -1, 0, 0, 1, -1]
        ternary_weight.scales[0] = 1.0

        # Create simple activations
        config = QuantConfig()
        activations_fp32 = np.array([2.0, 3.0, 1.0, 4.0], dtype=np.float32)  # [[2, 3], [1, 4]]
        quantized_activations = quantize_activations_int8(activations_fp32, len(activations_fp32), config)
        ...
        # [[2, 3], [1, 4]] * [[1, -1, 0], [0, 1, -1]] = [[2*1+3*0, 2*(-1)+3*1, 2*0+3*(-1)], [1*1+4*0, 1*(-1)+4*1, 1*0+4*(-1)]]
        # = [[2, -2+3, 0-3], [1, -1+4, 0-4]] = [[2, 1, -3], [1, 3, -4]]

        expected = [2.0, 1.0, -3.0, 1.0, 3.0, -4.0]

        print(f"✓ Expected: {expected}")
        print(f"✓ Got:      {[round(x, 1) for x in output]}")

        # Check if results are close (accounting for quantization)
        tolerance = 0.5  # Allow some tolerance due to quantization
        for i, (exp, got) in enumerate(zip(expected, output)):
            if abs(exp - got) > tolerance:
                print(f"✗ Mismatch at position {i}: expected {exp}, got {got}")
                return False
...
def main():
    print("=== BitNet Engine Component Tests (Pure Python) ===\n")

    success = True
    success &= test_quantization()
    success &= test_naive_matmul()
    success &= test_matmul_correctness()
